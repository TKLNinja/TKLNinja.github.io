...
/*:
 * @plugindesc We will build an ability acquisition system using armor system.
 * @author 尾角 つの (Tsuno Ozumi)
 *
 * @param Menu Switch ID
 * @desc It is the switch ID to judge whether to display it on the menu.
 * (0: always displayed -1: always hidden)
 * @default 0
 * 
 * @param Level UP Point
 * @desc Number of points to be acquired at the time of level up. (A: Actor)
 * @default a.level / 5 + 10
 *
 * @param Point Name
 * @desc System character of ability point.
 * @default ＡＰ
 
 * @param Point Name A
 * @desc System character (abbreviation) of the ability point.
 * @default AP
 *
 * @param Obtain Text
 * @desc It is a sentence at the time of availability point acquisition.
 * %1:Acquisition point %2: Point display name
 * @default Earn% 2 of% 1!
 *
 * @param Command Name
 * @desc Display name for the menu command.
 * @default Ability
 *
 * @param Point UP SE
 * @desc It is the setting of SE when the point rises.
 * SEName, volume, pitch, phase
 * @default Equip1,90,100,0
 *
 * @param Learning SE
 * @desc It is setting SE of ability when learning.
 * SEName, volume, pitch, phase
 * @default Item3,90,100,0
 *
 * @param Perch Padding
 * @desc It is the margin of the ability window on the ability screen.
 * @default 48
 *
 * @param Perch Stage Height
 * @desc It is the height of the stage of the ability tree on the ability screen.
 * @default 64
 * 
 * @help
 * 
 * Required plug-in:
 *    To use this plugin, you need Vitsuno_SpritePerch.js.
 *    on the plugin management screen, please place it so that it is below.
 *
 * Required image: Please put the following file in img / system.
 *    PerchCursor.png # Cursor image for image command (Horizontal square pattern)
 *    AbilityNetworkLearningMark.png # Learned mark image (16px x 16px)
 *
 * About this plug-in:
 *    This plug-in expresses abilities as armor instead of abilities.
 *    For example, the ability of ID: 1 refers to armor of ID: 1,
 *    If you learn ID: 1, you get the same effect as equipped with armor of ID: 1.
 *
 *    First of all, set the abilities (armor) ID as the starting point in the memoranda of the actor.
 *    Set the number of points necessary for learning to that ability · Set the next ability ID.
 *    I will set it for the next abilities the same way.
 *
 * Notes of actors and occupations:
 *    <abilityNetBases:ID1,ID2,ID3>  # Originability ability ID to be added
 *
 * Protector notes:
 *    <abilityNetPoint:num>          # Number of points required for learning
 *    <abilityNetNexts:ID1,ID2,ID3>  # Next ability ID
 *    <abilityNetBaseName:name>      # List display name at starting point abilities
 *
 *    <abilityNetPosition:x,y>       # Set the ability coordinates (optional)
 *
 * Notes of enemy characters:
 *    <dropAbilityNetPoint:num>      # Number of points to be acquired when you kill
 * 
 * Professional notes:
 *    <abilityNetworkId:ID>          # Occupation ID that shares abilities
 *
 * Plug-in command:
 *    AbilityNetwork GainActorPoint 'Actor-D' 'rising-value'  # Increase actor's points
 *    AbilityNetwork ClearActorAbility 'Actor-ID'     # Clear Actor's Ability
 *
 *    AbilityNetwork GoToScene 'Actor-party-order' # Move to the ability screen (order from 0)
 *
 * Other specifications and notes:
 *    For each occupation, the acquisition status of abilities and the number of possession points are saved.
 *    If the remaining ability level becomes negative, the acquired abilities are reset.
 *    Ability points do not decrease when level is down.
 *    Plug-in command screen move does not move if there are no characters in the order.
 *
 *    When abilities are selected, the destination of the cursor at the time of pressing up, down, left, and right is determined by the distance between abilities,
 *    Depending on the position, there may be abilities that can not be selected, so please test it thoroughly.
 *    If you can not select it, please adjust the number by increasing or decreasing the number of abilities, or changing the derivation.
 *    
 */
